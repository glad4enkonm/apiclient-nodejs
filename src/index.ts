...
import {
    ...
    UsersInterface,
    CreateManualCallListRequest,
    CreateManualCallListResponse,
    StartNextCallTaskRequest,
    StartNextCallTaskResponse,
    GetCallListsRequest,
    ...
    AppendToCallListRequest, AppendToCallListResponse
} from './Interfaces'
...
export default class VoximplantApiClient{
    ...
    */
    createManualCallList: (request:CreateManualCallListRequest):Promise<CreateManualCallListResponse> => {
      ...
      appendToCallList: (request:AppendToCallListRequest): Promise<AppendToCallListResponse> => {
          const reqMapper = [{rawName:'list_id', name:'listId', transformer: TypeTransformer.from('number')},
              {rawName:'file_content', name:'fileContent', transformer: TypeTransformer.to('string')},
              {rawName:'encoding', name:'encoding', transformer: TypeTransformer.to('string')},
              {rawName:'escape', name:'escape', transformer: TypeTransformer.to('string')},
              {rawName:'delimiter', name:'delimiter', transformer: TypeTransformer.to('string')}];
  ...
  public SmartQueue:SmartQueueInterface = {
    ...
    */
    ...
    /**
     * Adds a status if there is no match for the given internal status in agent_status_mapping and renames it if there is a match. It means that if the passed **sq_status_name** parameter is not in the smart_queue.agent_status_mapping table (smart_queue.agent_status_mapping#status_id = smart_queue.status#id), a new entry is created in there; if it is, the **name** field in its mapping is replaced with **custom_status_name**.
    */
    ...
    /**
     * Returns the list of sq-statuses (smart_queue.status) and custom statuses (smart_queue.agent_status_mapping) matches. If there is no match for an sq-status, only the name of this status is returned.
    */
    ...
    /**
     * Removes a mapping from smart_queue.agent_status_mapping. If there is no such mapping in the database, does nothing.
    */
